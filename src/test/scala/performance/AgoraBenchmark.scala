...
class AgoraBenchmark extends Bench.OfflineRegressionReport {
  ...
  val preferenceSet: ListSet[Candidate] =
    ListSet(Candidate("A"), Candidate("B"), Candidate("C"), Candidate("D"), Candidate("E"))
...
trait RuntimeRegression extends AgoraBenchmark {
  ...
  override def persistor: Persistor = new GZIPJSONSerializationPersistor(
    "target/benchmarks/borda/time"
  )

  performance.of("VotingMethod") in {
    measure
      .method(votingMethodName())
      .config(
        exec.benchRuns -> 15
      ) in {
      using(election) in { preferences =>
        votingMethod(preferences)
      }
    }
  }
...
trait MemoryRegression extends AgoraBenchmark {
  ...
  override def persistor: Persistor = new GZIPJSONSerializationPersistor(
    "target/benchmarks/borda/memory"
  )
