...
object Main {
  ...
  val parser = new scopt.OptionParser[Config]("compress"){
    ...
    note("""Possible values are as follows:""" + "\n" +
        ...
        """for -m:  EVACS, EVACSnoLP, EVACSDWD, Simple, Majority, Borda, Approval, Nanson, Kemeny-Young, Random-Ballot""" + "\n" +
  ...
  def main(args: Array[String]): Unit = {
    ...
    def callMethod(c: Config, election: List[WeightedBallot],  winnersfile:String, reportfile: String, candidates_in_order:  List[Candidate]) = {
      c.method match {
               ...
               case "Random-Ballot" => {
                 var r = RandomBallotMethod.runScrutiny(Election.weightedElectionToACTElection(election), candidates_in_order, c.nvacancies.toInt)
                 println("Scrutiny table for method Random-Ballot not implemented yet")
                 r.writeWinners(winnersfile)
               }
