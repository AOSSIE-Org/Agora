...
class RunAllMethods extends Specification {
  ...
  def test(method: String, electionFile: String, candidatesFile: String): Boolean = {
    Main.main(
      Seq(
        "-d", "files/Examples/", "-b", electionFile, "-c", candidatesFile, "-v", "2", "-m", method
      ).toArray
    )
    return true
  }
  ...
  "Agora" should {
    ...
    "execute EVACSnoLP" in {
      test("EVACSnoLP", "02-example.e", "02-candidates.txt") shouldEqual true
    }
    ...
    "execute Egalitarian" in {
      test("Egalitarian", "02-example.e", "02-candidates.txt") shouldEqual true
    }
    ...
    "execute Kemeny-Young" in {
      test("Kemeny-Young", "14-example.e", "14-candidates.txt") shouldEqual true
    }
    "execute Coomb" in { test("Coomb", "14-example.e", "14-candidates.txt") shouldEqual true }
    "execute Contingent" in {
      test("Contingent", "14-example.e", "14-candidates.txt") shouldEqual true
    }
    "execute InstantExhaustiveBallot" in {
      test("InstantExhaustiveBallot", "14-example.e", "14-candidates.txt") shouldEqual true
    }
    "execute RandomBallot" in {
      test("RandomBallot", "21-example.e", "21-candidates.txt") shouldEqual true
    }
    "execute PreferentialBlockVoting" in {
      test("PreferentialBlockVoting", "32-example.e", "32-candidates.txt") shouldEqual true
    }
    "execute HybridPluralityPreferentialBlockVoting" in {
      test(
