...
import org.aossie.agora.comparator.FishburnsExtension
import org.aossie.agora.comparator.KellyExtension
...
object SetComparator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def callMethod(
      ...
      c.method match {
        case "Kelly" =>
          KellyExtension.compare(
            election,
            candidates_in_order,
            ParameterParser.parse(c.directory + c.parameterFile)
          )

        case "Fishburns" =>
          FishburnsExtension.compare(
            election,
            candidates_in_order,
            ParameterParser.parse(c.directory + c.parameterFile)
          )
