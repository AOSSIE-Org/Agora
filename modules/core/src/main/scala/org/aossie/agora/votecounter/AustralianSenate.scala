...
import org.aossie.agora.votecounter.stv.TwoLastCandidatesForOneVacancy
import org.aossie.agora.votecounter.stv.SurplusDistribution
import org.aossie.agora.votecounter.stv.ACTBallot

sealed abstract class BulkExclusionType
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  def returnCandidateA(
    ...
    bulktype match {
      ...
      case SurplusDistributionBulk =>
        pickedTotals = totals.filter(p =>
          computeAdjustedNotionalVotes(p._1, totals, surplus) >= vacancyShortfall
        ) // Section 273 (13A)(a)
    }
    if (pickedTotals.nonEmpty) {
      ...
      if (pickedTotals.size != 1) {
        ...
        println(
          "More than one candidate satisfy conditions of Candidate A: Section 273 (13A)(a). One of them is picked: " + pickedTotals.head._1
        )
  ...
  def returnCandidateB(
    ...
    candidateA match {
      ...
      case None =>
        bulktype match {
          ...
          case SurplusDistributionBulk =>
            totalsOfCandidatesPotentiallyB = totals.filter(p =>
              computeAdjustedNotionalVotes(p._1, totals, surplus) < vacancyShortfall
            )
        }
      // val candidateB = totals.clone().filter(p => p._2 == valueOfCandidateB).head._1
    ...
    if (totalsOfCandidatesPotentiallyB.nonEmpty) {
      val orderedTotalsOfCandidatesPotentiallyB =
        totalsOfCandidatesPotentiallyB.toList.sortBy(_._2) // TODO: sort appropriately
      val candidatesB =
        for (
          (left, right) <-
            orderedTotalsOfCandidatesPotentiallyB.zip(orderedTotalsOfCandidatesPotentiallyB.tail)
          if computeNotionalVotes(left._1, totals) < right._2
        ) yield left
  ...
  def returnCandidateC(
    ...
    bulktype match {
      ...
      case SurplusDistributionBulk =>
        potentialCandidatesC =
          totals.filter(p => computeAdjustedNotionalVotes(p._1, totals, surplus) < leadingShortFall)
    }
  ...
  def selectCandidatesForBulkExclusion(
    ...
    bulktype match {
      ...
      case SurplusDistributionBulk =>
        val candidateA =
          returnCandidateA(totals, vacancyShortfall, SurplusDistributionBulk, surplus)
        println("candidateA: " + candidateA)
        candidateB =
          returnCandidateB(totals, candidateA, vacancyShortfall, SurplusDistributionBulk, surplus)
        println("candidateB: " + candidateB)
    }
    candidateB match {
      case Some(cB) => // "in a case where Candidate B has been identified"
        ...
        if (notionalVotesOfB < leadingShortfall) { // Section273 (13A)(c)
          ...
          bulktype match {
            ...
            case SurplusDistributionBulk =>
              candidateC =
                returnCandidateC(totals, leadingShortfall, SurplusDistributionBulk, surplus)
          }
          candidateC match {
          ...
          quotaReached(tls, result.getQuota) match {
              ...
              vacanciesFilled match {
                case false =>
                  ...
                  if (nws == numVacancies) { allWinners }
                    ...
                    winners(
                      newElection,
                      ccandidates.filterNot(allWinners.map(_._1).toSet.contains(_)),
                      numVacancies - nws
                    ) ::: allWinners
            ...
            case false =>
              ...
              if (newWinners.length == numVacancies) {
                ...
                winners(
                  newElection,
                  ccandidates.filterNot(x => candidatesToExclude.map(_._1).contains(x)),
  ...
  def exclusion(
    ...
    if (ws.nonEmpty) {
      val res = surplusesDistribution(
        newElection,
        ccandidates.filterNot(x => candidatesForExclusion.map(_._1).contains(x)),
