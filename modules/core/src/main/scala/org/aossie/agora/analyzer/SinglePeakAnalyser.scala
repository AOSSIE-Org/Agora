...
/** Please check the last page of the pdf
...
object SinglePeakAnalyser extends PreferenceAnalysisMethod[Candidate, Ballot] {
  ...
  /** recursive utility which finds the last ranked candidates and set L and set R and places those
    * candidates in left/right lists and then recurse
    * @param left
    * @param right
    * @param election
    * @param candidates
  ...
  def singlePeakAxisAux(
    ...
    if (candidates.isEmpty) {
      ...
      val B = election
        .filter(b =>
          b.preferences.nonEmpty && b.preferences.exists(c => !(left ++ right).contains(c))
        )
  ...
  /** returns true if there exists a voter who has x as last ranked candidate and also prefer
    * candidate l > x > r (algorithm reference : Set Nlxr)
    * @param election
    * @param x
    * @param l
    * @param r
  ...
  def existNrxl(
    ...
    if (l.nonEmpty && r.nonEmpty) {
      xLastRankedList.exists(prefs =>
        prefs.indexOf(l.get) < prefs.indexOf(x) && prefs.indexOf(x) < prefs.indexOf(l.get)
      )
  ...
  /** this checks if calculated axis is compatible with all the ballots as per definition 2 of
    * http://www.lamsade.dauphine.fr/~lang/papers/elo-ecai08.pdf
    * @param axis
    *   \- single peak ordering
    * @param election
    *   \- election
    * @return
    */
  def isCompatibleAxis(
    ...
    election.forall { b =>
      ...
      prefs.tail.zipWithIndex.forall(c1 => {
        prefs.tail.zipWithIndex
          .filter(c =>
            c._2 > c1._2 && {
              // check if c1 and c2 are on the same side of the axis
              if (
                axis.indexOf(prefs.head) < axis.indexOf(c1._1) && axis.indexOf(prefs.head) < axis
                  .indexOf(c._1)
                || axis.indexOf(prefs.head) > axis.indexOf(c1._1) && axis.indexOf(prefs.head) > axis
                  .indexOf(c._1)
              ) {
          ...
          .forall(c2 => {
            if (
              (axis.indexOf(peak) > axis.indexOf(c1._1) && axis.indexOf(c1._1) > axis
                .indexOf(c2._1)) ||
              (axis.indexOf(c2._1) > axis.indexOf(c1._1) && axis.indexOf(c1._1) > axis
                .indexOf(peak))
            ) {
              true
            } else {
