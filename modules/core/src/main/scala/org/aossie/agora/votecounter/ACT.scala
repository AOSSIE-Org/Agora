...
abstract class ACT
    ...
    with NewWinnersOrderedByTotals[Candidate, ACTBallot[Candidate]]
    with ACTSurplusDistributionTieResolution
    with ACTFractionLoss
    with ACTExclusion
    with ACTExclusionTieResolution[Candidate]
    with ACTExactWinnerRemoval[Candidate] {
  ...
  def declareNewWinnersWhileExcluding(
      candidate: Candidate,
      exhaustedBallots: Set[ACTBallot[Candidate]],
      newtotals: Map[Candidate, Rational],
      totalsWithoutNewWinners: Map[Candidate, Rational],
      newElectionWithoutFractionInTotals: Election[Candidate, ACTBallot]
  ...
  def filterBallotsWithFirstPreferences(
    ...
    var ballots: List[ACTBallot[Candidate]] = List()
    for (b <- election)
          ...
          case true =>
            ...
            vacanciesFilled match {
              case false =>
                ...
                val newElection: Election[Candidate, ACTBallot] = res._1
  ...
  def extractMarkings(election: Election[Candidate, ACTBallot]): Set[Int] = {
  ...
  def exclusion(
    ...
    var exhaustedBallots: Set[ACTBallot[Candidate]] = Set()
