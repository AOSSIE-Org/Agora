...
object KellyExtension extends SetExtensionMethods[Candidate, Ballot] with LazyLogging {
  ...
  override def compare(
    ...
    require(
      parameters.comparisonSets.isDefined && parameters.comparisonSets.get.set1.forall(c =>
        candidates.exists(cand => cand.name == c)
      ) &&
  ...
  /** X PK Y ⇔ ∀x∈X, y∈Y: (x R y) ∧ ∃x∈X, y∈Y: (x P y) => set X is Kelly preferred to set Y
    * @param matrix
    *   matrix of pairwise votes between candidates
    * @param setX
    * @param setY
    * @return
    */
  def kellyComparison(
    ...
    val xRy = setX.forall(cand1 =>
      setY.forall(cand2 => {
        matrix(candidates.indexOf(cand1))(candidates.indexOf(cand2)) >= matrix(
          candidates.indexOf(cand2)
        )(candidates.indexOf(cand1))
      })
    )

    xRy && setX.exists(x =>
      setY.exists(y =>
        matrix(candidates.indexOf(x))(candidates.indexOf(y)) > matrix(candidates.indexOf(y))(
          candidates.indexOf(x)
        )
      )
    )
